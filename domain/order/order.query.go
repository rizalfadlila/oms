...
const (
	queryGetReportOrder = `select CONCAT(c.first_name, CONCAT(' ', c.last_name)) as customer_name, CONCAT(e.first_name, CONCAT(' ', e.last_name)) as employee_name, s.shipping_method
									from orders o
	...
	queryGetReportOrderDetail = `select p.id as product_id, p.product_name, od.quantity, od.unit_price, od.discount 
									...
									inner join products p on p.id = od.product_id and p.is_deleted = 0
